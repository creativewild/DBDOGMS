...
bot.on("message", function(message){
  ...
  if(message.content.startsWith("+") & message.channel.isPrivate === false) {
    ...
    if( infoArray.length != 6 ) {
      ...
      message.reply("You've entered your datad with either too many or too little variables. That or it just did not make sense. Please type !Help for more information.").then(bot.deleteMessage(message).catch(function(err){console.log("Error With Delete + Message Process: "+err)}));
        ...
        db.findOne( { _id: message.author.id }, function( err, result ) {
          if( result ) {
            ...
            message.reply( "You're aleady in the database, no need to use the register command again player ***" + char.familyName + "***").then(bot.deleteMessage(message).catch(function(err){console.log("Error With Delete + Message Process: "+err)}))
          } else if ( !err ){
  ...
  var rmvRoles = function(rlname) {
    bot.removeMemberFromRole(message.author.id, message.server.roles.get("name", rlname), function ( err ) {message.channel.sendMessage( "```"+err.stack+"```")})
  }
  ...
  if(message.content === "setDev" & message.author.id === "92738910362292224") {
    ...
    bot.user.setStatus("active", "Persist mode. Notify Dr. Gray of downtime/issues.").catch(function(err){console.log("error with status setting 2: "+ err)})
    console.log("Persistent Mode Set.")
  }

  if(message.content === "C") {
    if(message.member.roles.get("name", "Hoodrat") === true){
      console.log("pass")
    } else {
      console.log("fail")
    }
